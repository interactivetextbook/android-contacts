...
fun CountryCodeBottomSheet(
    ...
    CommonBottomSheet(
        title = stringResource(id = R.string.filter_by_country_code),
        ...
        CountryCodeOption(
            ...
            onCountryCodeSelected = {
                val newSelected = selectedCodes.toMutableSet()
                if (isSelected) {
                    newSelected.remove(countryCode)
                } else {
                    newSelected.add(countryCode)
                }
                onCodesSelected(newSelected)
            }
        )
    }
}
...
private fun CountryCodeOption(
    ...
    Row(
        ...
        Checkbox(
            checked = isSelected,
            onCheckedChange = { _ -> onCountryCodeSelected() }
        )
